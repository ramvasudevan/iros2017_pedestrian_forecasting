...
with open(join("config", "scene_order.json")) as f:
    st = f.read()
json_acceptable_string = st.replace("'", "\"")
...
folders = [ os.join(x, "{}", "{}").format(scene_name, split_index) for x in dic['folders']]
labels = dic['labels']
print folders[0]
...
fnames = [[np.load(join(folders[0], "pr_agent_{}_time_{}.npy").format(agt, t)) for t in timer[agt]] for agt in range(len(all_data[scene_number][split_index][1]))]
